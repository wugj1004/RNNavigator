...
export default class RNAsyncStorage extends Component{
        ...
        if(storage==undefined){
            storage = new Storage({
                ...
                /* 如果storage中没有相应数据，或数据已过期，则会调用相应的sync方法，无缝返回最新数据。
                 sync方法的具体说明会在后文提到你可以在构造函数这里就写好sync的方法
                 或是写到另一个文件里，这里require引入或是在任何时候，直接对storage.sync进行赋值修改*/
    ...
    static _savaExpires(key,object,expires){
    ...
    static _save(key,object){
        this._savaExpires(key,object,defaultExpires);
    }
